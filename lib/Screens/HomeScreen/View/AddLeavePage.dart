    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text("${'add'.tr} ${'leave'.tr}",style: const TextStyle(color:Color(0xFF5b1aa0),fontSize: 15),),
          centerTitle: true,
        ),
        body: Padding(
                  ...
                  children: [
                              ...
                              await homeController.checkLeaveTypeDataAvailable(leaveTypeDataModel: e).then((value) async {
                                ...
                                else
                                {
                                  ...
                                  Timer(const Duration(milliseconds: 200), () {
                                    EasyLoading.showError("${'sorry'.tr}  ${e.leaveTypeName} ${'leave'.tr} ${'quota'.tr.toLowerCase()} ${'is'.tr.toLowerCase()} ${'finished'.tr.toLowerCase()}");
                                  });
                                }
                              });
                            }
                            ...
                            child: TextFormField(
                              ...
                              onTap: () async {
                                ...
                                if(dateTime != null)
                                {
                                  ...
                                  {
                                    EasyLoading.showError("please_select_valid_date".tr);
                                  }
                                }
                              },
                            ),
                              ...
                              onTap: () async {
                                ...
                                if(dateTime != null)
                                {
                                  if((dateTime.compareTo(DateTime(DateTime.parse(homeController.startDate.value).year,DateTime.parse(homeController.startDate.value).month,DateTime.parse(homeController.startDate.value).day)) == 1))
                                  {
                                    ...
                                    {
                                      EasyLoading.showError("${'please_select_valid_date'.tr} ${homeController.dropDownLeaveType.value} ${'leave'.tr.toLowerCase()} ${'max'.tr} ${homeController.canAddEdit.value == 0 ? homeController.leaveTypeDataAvailableList.isNotEmpty ? homeController.vadhelaLeaveTypeDays.value : homeController.leaveTypeOneData.value.leaveNos : (DateTime(DateTime.parse(homeController.leaveOneData.value.endDate!).year,DateTime.parse(homeController.leaveOneData.value.endDate!).month,DateTime.parse(homeController.leaveOneData.value.endDate!).day).difference(DateTime(DateTime.parse(homeController.leaveOneData.value.startDate!).year,DateTime.parse(homeController.leaveOneData.value.startDate!).month,DateTime.parse(homeController.leaveOneData.value.startDate!).day)).inDays) + (homeController.leaveTypeDataAvailableList.isNotEmpty ? homeController.vadhelaLeaveTypeDays.value : int.parse(homeController.leaveTypeOneData.value.leaveNos!))} ${'days'.tr} ${'available'.tr}");
                                    }
                                  }
                                  else
                                  {
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(20)),
                        labelText: "${'leave'.tr} ${'reason'.tr}",
                      ),
                      textAlign: TextAlign.start,
                      maxLines: 3,
                      validator: (value) {
                        if (value!.isEmpty) {
                          return "${'please'.tr} ${'fill'.tr} ${'leave'.tr} ${'reason'.tr}...";
                        }
                      },
                    ),
        ...
        bottomNavigationBar: InkWell(
          onTap: () async {
            ...
            {
              EasyLoading.showError("${'sorry'.tr} ${'please'.tr} ${'select'.tr} ${'leave'.tr} ${'type'.tr}...");
            }
            else if(key.currentState!.validate())
            {
              if(homeController.imagePath.isEmpty)
              {
                EasyLoading.showError("${'sorry'.tr} ${'please'.tr} ${'take_photo'.tr}...");
              }
              else
              {
                showDialog(context: context, builder: (context) {
                  return AlertDialog(
                    title: Text("${'save'.tr}?"),
                    content: Text("${'are_you_sure_want_to'.tr} ${'save'.tr.toLowerCase()} ${'this'.tr.toLowerCase()} ${'leave'.tr}?"),
                    actions: [
                      ElevatedButton(
                          onPressed: () async {
                            if(homeController.canAddEdit.value == 0)
                            {
                              ...
                              if(connectivityResult != ConnectivityResult.none) {
                                ...
                                await ApiHelper.apiHelper.insertLeaveData(
                                  ...
                                  EasyLoading.showSuccess('${'save'.tr} ${'successfully'.tr}');
                                });
                              }
                              else
                              {
                            ...
                            else
                            {
                              ...
                              if(connectivityResult != ConnectivityResult.none)
                              {
                                await ApiHelper.apiHelper.updateOneLeaveData(leaveDataModel: LeaveDataModel(
                                  ...
                                  Get.back();
                                  homeController.leaveData.value = (await ApiHelper.apiHelper.getLeaveDatauserWise(user_id: loginController.UserLoginData.value.id!))!;
                                  EasyLoading.showSuccess('${'updated'.tr} ${'successfully'.tr}');
                                });
                              }
                              else
                              {
          ...
          child: IntrinsicHeight(
            child: Container(
              ...
              child: Obx(
                    () => Text(
                  homeController.canAddEdit.value == 0 ? "submit".tr : "edit".tr,
                  style: const TextStyle(
